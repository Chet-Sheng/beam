...
def run(argv=None, save_main_session=True):
  ...
  pipeline_options = PipelineOptions(pipeline_args)  # [star] set various options for our pipeline
  pipeline_options.view_as(SetupOptions).save_main_session = save_main_session
  with beam.Pipeline(options=pipeline_options) as p:  # [star] create a Pipeline object with the options we’ve just constructed

    # Read the text file[pattern] into a PCollection.
    lines = p | ReadFromText(known_args.input)  # [star] Each element in the output PCollection represents one line of text from the input file.

    # Count the occurrences of each word.
    counts = (
        lines
        | 'Split' >> (beam.FlatMap(lambda x: re.findall(r'[A-Za-z\']+', x))  # [star] This transform splits the lines in PCollection<String>, where each element is an individual word in Shakespeare’s collected texts.
    ...
    output = counts | 'Format' >> beam.Map(format_result)   # [star] transform formats each of the key/value pairs of unique words and occurrence counts into a printable string suitable for writing to an output file.

    # Write the output using a "Write" transform that has side effects.
    # pylint: disable=expression-not-assigned
    output | WriteToText(known_args.output)  # [star] This transform takes the final PCollection of formatted Strings as input and writes each element to an output text file
